...
class LSTMModel(nn.Module):
    ...
    - nlayers:                number of LSTM layer(s)
    ...
    def __init__(self,
        ...
        self.rnn = nn.LSTM(self.input_vector_size, self.embed_dim, self.nlayers, \
                    batch_first=True, dropout=self.dropout, bidirectional=False, device=device, dtype=floatType)
        # Decoder
    ...
    def summary(self) -> None:
        ...
        """
        self.log.emptyLines()
        self.log.info("Model architecture: LSTM")
        string = f" {'=' * 90}\n\t"
        ...
        string += f"LSTM layers:             {self.nlayers} layer(s) of hidden size [{self.embed_dim}]\n\t"
